...
- `VECTOR_DB_URL` & `VECTOR_DB_API_KEY`: Vector database instance credentials
- `EMBED_ENDPOINT` & `OPENAI_API_KEY`: Your embedding service
- `BLOB_READ_WRITE_TOKEN`: Vercel Blob token
- `EMAIL_API_KEY`: Email service API key for sending (production)
...
For production email sending, configure your email service:

1. Get your email service API key from your provider's dashboard
...
- **Vector Search**: Separate collection per brand (`brand_${slug}`)
...
3. Set up vector database instance
...
- ✅ `/api/ingest/embed` writes to brand's vector collection
